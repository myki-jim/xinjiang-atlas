...
function renderRoutePanel() {
  ...
  const posterDownload = routeState.days === 7 && routeState.style === 'slow' && routeState.mode === 'drive' && routeState.pace === 'full'
    ? `<a class="fit-route poster-download" href="${import.meta.env.BASE_URL}exports/pamir-shache-7-day-road-trip.png" download="帕米尔与莎车-七日行进图.png">↓ 下载完整行进图 · 10000 × 6600</a>` : '';
  const nightGuide = routeState.days === 7 && routeState.style === 'slow' ? `<section class="route-night"><span class="eyebrow">✦ NIGHT SKY / 星空与帐篷</span><h3>先选取景点，再确认营地。</h3>
    <p><strong>D3 · 喀拉库勒湖：</strong>雪峰与湖面最适合当这条线的星空前景。如需在那里过夜，须把原定的塔县住宿改为获许可营地；景区内有不周山营地，但<strong>自带帐篷能否搭设和夜间拍摄，要先问经营方</strong>。不安排拍完星空后赶夜路。</p>
    <p><strong>D4 · 塔县周边：</strong>保留原行程时更从容，可在明确允许的营地或住宿地夜拍。班迪尔蓝湖周边山区禁止户外露营；勿把观景点当成扎营点。</p>
    <p>${routeState.start === '2026-10-01' ? '按 10 月 1 日出发：10 月 3 日下弦，后半程月光渐弱；是否能拍到星空仍取决于云量、风和当晚月出时刻。' : '出发日变化后，请重新查看当天月相、月出、云量和风况。'}高海拔夜间注意保暖与身体反应。</p>
    <div class="route-night-links"><a href="https://www.xjakt.gov.cn/xjakt/c104939/202508/aad4037e56714e7ea7f33c12d1388df0.shtml" target="_blank" rel="noopener noreferrer">阿克陶露营公告 ↗</a><a href="https://www.btzx.com.cn/web/2026/5/3/ARTI1777778943656676.html" target="_blank" rel="noopener noreferrer">塔县露营公告全文 ↗</a><a href="https://eclipse.gsfc.nasa.gov/SKYCAL/SKYCAL.html?cal=2026" target="_blank" rel="noopener noreferrer">2026 月相 ↗</a></div></section>` : '';
  ...
  panel.innerHTML = `<div class="route-scroll"><div class="route-head"><div><span class="eyebrow">THE FIELD ROUTE / SOUTH XINJIANG</span><h2>选一条路，<br><em>去看更大的风景。</em></h2></div><button id="close-route" class="close-detail" aria-label="关闭路线规划">×</button></div>
    ...
    <div class="route-summary"><span class="eyebrow">RECOMMENDED JOURNEY</span><h3>${esc(plan.name)}</h3><p>${esc(plan.tone)}</p><div class="route-metrics"><span><strong>${plan.days}</strong> 天</span><span><strong>${scenicCount}</strong> 个地图停靠点</span><span>喀什出发 · 返回喀什</span></div><p class="route-intro">${esc(plan.intro)}</p><button id="fit-route" class="fit-route">⌖ 在地图上看完整顺序</button>${posterDownload}</div>
    <p class="route-mode-note">${routeState.mode === 'drive' ? '喀什取还车；高原、盘龙和西极段都需要实地核查天气与道路。' : routeState.mode === 'rail' ? '铁路用于抵达喀什；莎车方案的喀什—莎车城际段也可乘火车。帕米尔和西极没有可替代的景点间铁路，需当地车辆。' : '飞机用于往返喀什；景点之间仍需租车或包车，没有把短途航班虚构为景区接驳。'}${routeState.pace === 'full' ? ' 多点摄影增加顺路观景点，依现场开放与停车条件取舍。' : ''}</p>${nightGuide}
